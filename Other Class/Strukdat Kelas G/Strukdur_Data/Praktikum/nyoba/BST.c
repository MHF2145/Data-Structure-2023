...
int bst_fparent(BSTNode *bst, int value, int parent)
{
    BSTNode *temp = bst;
    if (temp != NULL)
    {
        ...
        if (value == temp->key)
            ...
            return bst_fparent(temp->right, value, temp->key);
        else if (value < temp->key)
            return bst_fparent(temp->left, value, temp->key);
    }
    return -1;
}
...
int main()
{
    ...
    bst_insert(&set, 3);
    bst_insert(&set, 7);
    printf("Parent : %d\n", bst_fparent(set._root, 6, -1));
    printf("Sibling : %d\n", bst_fsibling(set._root, 12));
    // bst_inorder(&set);
    // puts("");
