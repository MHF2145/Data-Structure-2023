...
int id = 0; 
void cariIndex(AVLNode *root){
	if (root){
		cariIndex(root->right); 
		root->idx = ++id;
        cariIndex(root->left); 
	} 
}

int searchNodeByIndex(AVLNode *node, int value){
    ...
    while (t--){
		...
		if (str == "Cari"){
            ...
            if (avl_find(&avlku, numb)){
				...
				int temp = searchNodeByIndex(avlku._root, numb); 
                cout << "Kasetnya ada di tumpukan ke - " << temp << endl; 
                continue; 
            }
			...
			int numb; 
			cin >> numb; 
			cout << "AKU TUH BINGUNG HARUS NGAPAIN BEB!" << endl;
		}
