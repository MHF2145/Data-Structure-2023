...
public:
    ...
    int distanceFromRoot(AVLNode* root, int value) {
        if (value == root->data) {
            return 0;
        }
        else if (value < root->data) {
            return abs(root->data - root->left->data) + distanceFromRoot(root->left, value);
        }
        else {
            return abs(root->data - root->right->data) + distanceFromRoot(root->right, value);
        }
    }

    int distanceFromNode(AVLNode* root, int a, int b) {
        if (root == nullptr) {
            return 0;
        }
    ...
    int dfn(int a, int b) {
        return distanceFromNode(_root, a, b);
    }
};
