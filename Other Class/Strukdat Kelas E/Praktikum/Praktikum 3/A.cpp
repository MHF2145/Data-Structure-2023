...
//void alone(AVL& tree, char value);
...
private:
    ...
    void _inorder(AVLNode *node) {
        if (node) {
            _inorder(node->left);
            printf("%c ", node->data); //DIGANTI (karena dimintanya karakter)
            _inorder(node->right);
        }
    }
...
int main(int argc, char const *argv[])
{
    ...
    if (set.isEmpty())
        cout << "Tidak ada yang sendiri" << endl;
    else {
        cout << "Mereka sendirian: ";
        set.inorder();
    }
