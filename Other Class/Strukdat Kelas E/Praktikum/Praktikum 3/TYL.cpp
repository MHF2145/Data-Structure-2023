...
//fungsi buat nyari parent dari sebuah node
AVLNode* avl_parent(AVLNode* root, int key){
    AVLNode* node = avl_find(root, key);
    if (node == NULL) {
        return NULL; 
    }
    else {
...
int main()
{
    ...
    while(q--){
        cin >> x;
        AVLNode *node = avl_find(root, x); //cari node yang sesuai
        if(node){ //kalo ditemukan
            AVLNode *parent = avl_parent(root, x); //cari parentnya
            if (parent == NULL) {
                cout << "Dia si tuyul pertama" << " " << x << endl; //kalo dia gapunya parent, berarti dia node root
            }
            else {
                cout << parent->data << " " << x << endl; //kalo dia ada parent, print parentnya
            }
        }
        else{
            root = _insert_AVL(root, NULL, x); //kalo gaada node yang ditemuin, masukin node baru trs balikin root yang updated
        }
    }
